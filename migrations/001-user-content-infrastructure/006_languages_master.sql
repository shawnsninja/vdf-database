-- =====================================================================================
-- Module 1: User & Content Infrastructure - languages_master table
-- Version: 2.1
-- Description: Defines all languages supported or planned for the platform, providing
--              canonical language codes, display names, and configuration
-- Dependencies:
...
CREATE TABLE public.languages_master (
    ...
    -- Constraints
    CONSTRAINT check_language_code_format 
        CHECK (language_code ~ '^[a-z]{2}(-[A-Z]{2})?$' 
               AND char_length(language_code) >= 2 
               AND char_length(language_code) <= 5),
    
    CONSTRAINT uq_languages_master_display_order_ui 
        UNIQUE (display_order_ui)
);

-- Unique Partial Index to ensure only one language is the primary content language
